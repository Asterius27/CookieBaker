...
# specifically focusing on CookieHunter's findings and disregarding CodeQL results.
# It processes zipped log files within a specified directory, categorizes URLs and generates summary statistics.
#
...
#   Example: python3 getStatsFromZipLog_only_ch.py -i cookie_hunter/logs/1_logs_flask_dockercompose/
...
PATTERN_STARTING_LOGIN_URLS = r"Starting login @ ([ -~]*)"
PATTERN_LOGIN_ATTEMPT_CH = r"\[LoginManager\](?:.*)Starting login @ ([ -~]*)"
PATTERN_LOGIN_ATTEMPT_CH_ORACLE = r"\[LoginManager\] \[ORACLE\] Starting login @ ([ -~]*)"
PATTERN_SIGNUP_ATTEMPTED_URLS = r"Starting signup fuzzing @ ([ -~]*)"

PATTERN_LOGIN_URLS_CODEQL_WITH_FORMS = r"\[COOKIEBAKER\]\[DEBUG\] Login Endpoints CodeQL with forms: (\[.*\])"  # dai log prendi group 2
PATTERN_SIGNUP_URLS_CODEQL_WITH_FORMS = r"\[COOKIEBAKER\]\[DEBUG\] Signup Endpoints CodeQL with forms: (\[.*\])"  # dai log prendi group 2
...
lstNameRepoLoginInBothCHAndCodeQL = []
lstNameRepoSignupInBothCHAndCodeQL = []
...
for root, dirs, files in os.walk(base_dir):
    ...
    for file in files:
        ...
        if file.endswith('.zip'):
            ...
            if not containsPort(file, "27017") and not containsPort(file, "5672"):  # exclude mongo
                ...
                with zipfile.ZipFile(zip_path, 'r') as myzip:
                    ...
                    if 'out.log' in myzip.namelist():
                        with myzip.open('out.log', 'r') as mylog:
                            ...
                            (_, numLoginURLsCookieHunter) = extractNumFromText(PATTERN_NUM_LOGIN_URLS_COOKIEHUNTER, logStr)
                            ...
                            (_, numLoginURLsCodeQlReachable) = extractNumFromText(PATTERN_NUM_LOGIN_URLS_CODEQL_REACHABLE,
                                                                                  logStr)
                            ...
                            (_, numSignupURLsCodeQlReachable) = extractNumFromText(PATTERN_NUM_SIGNUP_URLS_CODEQL_REACHABLE,
                                                                                   logStr)
                            ...
                            hasAnyLoginFromCodeQL = len(lstLoginURLsCodeQlReachable) > 0
                            hasAnySignupFromCodeQL =len(lstSignupURLsCodeQlReachable) > 0
                            hasAnyLoginFromCH = len(lstLoginURLsCookieHunterReachable) > 0
                            hasAnySignupFromCH = len(lstSignupURLsCookieHunterReachable) > 0
                            ...
                            areThereBothLoginAndSignupReachableOnSamePort = \
                                len(lstLoginURLsCookieHunterReachable) > 0 \
                                and len(lstSignupURLsCookieHunterReachable) > 0
                            ...
                            _, (fuzzerDomain, fuzzerUrl, fuzzerSuccess) = extractFirstMatchAsTuple(PATTERN_FUZZER_SIGNED_UP,
                                                                                                   logStr)
                            ...
                            isLoginCHAttemptedOracle = isTherePatternInText(PATTERN_LOGIN_ATTEMPT_CH_ORACLE, logStr)

                            lstSignupURLsCodeQlReachable = []
                            lstLoginURLsCodeQlReachable = []
                            ...
                            _, loginAttemptsUrlsOracle = extractAllGroupsFromText(PATTERN_LOGIN_ATTEMPT_CH_ORACLE, logStr)
                            loginAttemptsUrlsOracle = removePortsFromUrls(loginAttemptsUrlsOracle)

                            isLoginAttempted = (isLoginCHAttempted \
                                               and areAnyElementsOfList1InList2(loginAttemptsUrls,lstLoginURLsCookieHunterReachable) \
                                                and isSignupUrlInCookieHunter) \
                                               or (isLoginCHAttemptedOracle and areAnyElementsOfList1InList2(loginAttemptsUrlsOracle,lstLoginURLsCookieHunterReachable))

                            isLoginSuccessful = isLoginUrlInCookieHunter and not isLoginCredentialsUrlInCodeQl \
                                                and not isLoginCredentialsUrlInCookieHunter and not isLoginUrlInCodeQl \
                                                and isSignupUrlInCookieHunter


                            _, signupAttemptsUrls = extractAllGroupsFromText(PATTERN_SIGNUP_ATTEMPTED_URLS,logStr)
                            signupAttemptsUrls = removePortsFromUrls(signupAttemptsUrls)

                            isSignupAttempted = areAnyElementsOfList1InList2(signupAttemptsUrls,lstSignupURLsCookieHunterReachable) \
                                                and len(lstLoginURLsCookieHunterReachable) # (there are CH login urls)

                            isLoginSuccessfulThanksToSignup = isTherePatternInText(PATTERN_LOGIN_SUCCESSFUL,logStr) \
                                                              and isSignupUrlInCookieHunter and not isSignupUrlInCodeQl \
                                                              and not lmSuccess
                            isLoginSuccessful = isLoginSuccessful or isLoginSuccessfulThanksToSignup
                            ...
                            counts.append((file, numLoginURLsCookieHunter, numLoginURLsCookieHunterReachable,
                                           ...
                                           # se cerco CH + credenziali, se è false, allora non incremento contatore Login form perche vuol dire che logins urls arrivano da codeql (idem con patate per gli altri is...)
                                           isLoginUrlInCodeQl,
                                           isLoginCredentialsUrlInCookieHunter,
                                           isLoginCredentialsUrlInCodeQl,
                                           isSignupAttempted,

                                           hasAnyLoginFromCodeQL,
                                           hasAnySignupFromCodeQL,
                                           hasAnyLoginFromCH,
                                           hasAnySignupFromCH
                                           ))
    ...
    repoHasLoginInBothCHAndCodeQL = False
    repoHasSignupInBothCHAndCodeQL = False
    ...
    for (file, numLoginURLsCookieHunter, numLoginURLsCookieHunterReachable,
        ...
        repoHasLoginInBothCHAndCodeQL = repoHasLoginInBothCHAndCodeQL or (hasAnyLoginFromCH and hasAnyLoginFromCodeQL)
        repoHasSignupInBothCHAndCodeQL = repoHasSignupInBothCHAndCodeQL or (hasAnySignupFromCH and hasAnySignupFromCodeQL)
    ...
    if repoHasLoginInBothCHAndCodeQL:
        lstNameRepoLoginInBothCHAndCodeQL.append(repoName)
    if repoHasSignupInBothCHAndCodeQL:
        lstNameRepoSignupInBothCHAndCodeQL.append(repoName)
...
outputGlobalStats = open(f"{DEFAULT_FILENAME_FOR_STATS}_global_only_CH.txt", "w")
...
lstNameRepoSignupInBothCHAndCodeQL.sort(key=str.casefold)
lstNameRepoLoginInBothCHAndCodeQL.sort(key=str.casefold)
...
outputGlobalStats.write(f"Num repos with Signup in both CH and CodeQL: {len(lstNameRepoSignupInBothCHAndCodeQL)}\n"
                        f" - {lstNameRepoSignupInBothCHAndCodeQL}\n")
outputGlobalStats.write(f"Num repos with Login in both CH and CodeQL: {len(lstNameRepoLoginInBothCHAndCodeQL)}\n"
                        f" - {lstNameRepoLoginInBothCHAndCodeQL}\n")
