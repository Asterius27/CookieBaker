...
#   Example: python3 getStatsFromZipLog_ch_credentials.py -i cookie_hunter/logs/1_logs_flask_dockercompose/
...
PATTERN_STARTING_LOGINCREDENTIALS_URLS = r"\[LoginWithCredentials\](?:.*)Starting login @ ([ -~]*)"
...
for root, dirs, files in os.walk(base_dir):
                ...
                with zipfile.ZipFile(zip_path, 'r') as myzip:
                    ...
                    if 'out.log' in myzip.namelist():
                        with myzip.open('out.log', 'r') as mylog:
                            ...
                            # in this script only CH+credentials, so we ignore codeql urls
                            lstLoginURLsCodeQl = []
                            lstSignupURLsCodeQl = []
                            ...
                            _, loginAttemptsWithCredentialsUrls = extractAllGroupsFromText(
                                PATTERN_STARTING_LOGINCREDENTIALS_URLS, logStr)
                            loginAttemptsWithCredentialsUrls = removePortsFromUrls(loginAttemptsWithCredentialsUrls)

                            isLoginAttempted = (areAnyElementsOfList1InList2(loginAttemptsUrls, lstLoginURLsCookieHunterReachable) \
                                                and not areAllElementsOfList1InList2(loginAttemptsUrls, lstLoginURLsCodeQlReachable) \
                                                and isSignupUrlInCookieHunter) \
                                               or (areAnyElementsOfList1InList2(loginAttemptsWithCredentialsUrls, lstLoginURLsCookieHunterReachable))

                            isLoginSuccessful = (isLoginCredentialsUrlInCookieHunter or isLoginUrlInCookieHunter) \
                                                and not isLoginCredentialsUrlInCodeQl and isLoginAttempted
                            ...
                            isSignedUp = isSignupUrlInCookieHunter and not isSignupUrlInCodeQl

                            isLoginCookieCollected = isTherePatternInText(PATTERN_LOGIN_SUCCESSFUL, logStr)

                            isLoginSuccessfulThanksToSignup = isLoginCookieCollected and isSignupUrlInCookieHunter and not isSignupUrlInCodeQl and not lmSuccess
                            isLoginSuccessful = (isLoginSuccessful or isLoginSuccessfulThanksToSignup) \
                                                and isLoginCookieCollected
    ...
    for (file, numLoginURLsCookieHunter, numLoginURLsCookieHunterReachable,
        ...
        # CH + Credentials => no use of CodeQL urls!
...
outputGlobalStats = open(f"{DEFAULT_FILENAME_FOR_STATS}_global_CH_CREDENTIALS.txt", "w")
