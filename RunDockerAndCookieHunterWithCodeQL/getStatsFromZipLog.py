...
#   Example: python3 getStatsFromZipLog.py -i cookie_hunter/logs/1_logs_flask_dockercompose/
...
# These patterns are for extracting the comparison result of visited and reachable URLs and
# the full set of urls with their respective status
...
# These patterns are used for checking if the crawling process is attempted or successful
...
# base_dir = 'cookie_hunter/logs/testing'
repoResults = []
urlsRepo = []
...
for root, dirs, files in os.walk(base_dir):
                ...
                with zipfile.ZipFile(zip_path, 'r') as myzip:
                    ...
                    if 'out.log' in myzip.namelist():
                        with myzip.open('out.log', 'r') as mylog:
                            ...
                            # check if signup attempted, login attempted and login successful
                            isLoginAttempted = isTherePatternInText(PATTERN_LOGIN_ATTEMPTED, logStr)
    ...
    for (file, numLoginURLsCookieHunter, numLoginURLsCookieHunterReachable,
         ...
         isLoginAttempted, isLoginSuccessful, areThereBothLoginAndSignupReachableOnSamePort, isSignupAttempted
...
outputGlobalStats = open(f"{DEFAULT_FILENAME_FOR_STATS}_global.txt", "w")
