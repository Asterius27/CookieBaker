...
where f = FlaskLogin::getFunctionViews()
	...
	and not decorator.(Call).getFunc().(Attribute).getObject() = Flask::FlaskApp::instance().getAValueReachableFromSource().asExpr()
    and not decorator.(Call).getFunc().(Attribute).getObject() = API::moduleImport("flask").getMember("Blueprint").getACall().getReturn().getAValueReachableFromSource().asExpr()
