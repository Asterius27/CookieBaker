...
4.  **Merge candidate credentials:** Use the `credentialMerger.py` script to merge extracted credentials in a single JSON file.
    ```bash
     python credentialMerger.py -i <path/to/file1.json> <path/to/file2.json> ...
    ```
...
To avoid running the analysis multiple times on the same set of repositories, 4 different scripts to extract stats from Cookie Hunter logs are available: 
